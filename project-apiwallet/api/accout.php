<?php
   ...
   if($api_req == "GET"){
       if($id != 0){
           ...
           // Call API : users/
           $data = $db->getAllAccouttype(); // $data is array
       }
    ...
    if($api_req == "POST"){
        //$cid = $_POST["cid"];
        $name   = $_POST["name"];
        if($db->newAccouttype($cid,$name)){
            echo $db->message("Add New Accout Sucessfully.",false);
        }else{
            echo $db->message("Failed to New Accout!!!",true);
        }
    }

    if($api_req == "DELETE"){
        if($id != 0){ // Have id value from Call request API
            if($db->delAccouttype($id)){ //DELETE Success.
                echo $db->message("Delete Accouttype Successfully.",false); 
            }else{
                echo $db->message("Failed to Dedete Accouttype.",true);
            }
        }else{
